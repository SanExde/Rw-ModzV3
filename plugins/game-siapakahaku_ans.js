...
handler.before = async function (m) {
    ...
    if (!(id in this.siapakahaku)) return conn.sendButton(m.chat, 'Soal itu telah berakhir', author, ['SIAPA KAH AKU? ', '/siapakahaku'], m)
    if (m.quoted.id == this.siapakahaku[id][0].id) {
        ...
        if (m.text.toLowerCase() == json.jawaban.toLowerCase().trim()) {
            ...
            conn.sendButton(m.chat, `*Benar!*\n+${this.siapakahaku[id][2]} XP`, author, ['SIAPA KAH AKU?', '/siapakahaku'], m)
